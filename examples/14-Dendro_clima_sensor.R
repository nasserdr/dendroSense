...
# get the file 2020______________________________________________________________________________________________________
setwd("~/mnt/Data-Work-RE/26_Agricultural_Engineering-RE/263_DP/03_Persoenliche_Unterlagen/Wata/08-R_dendro_files/Bidaux/2020")
#odir = "~/mnt/Data-Work-RE/26_Agricultural_Engineering-RE/263_DP/03_Persoenliche_Unterlagen/Wata/08-R_dendro_files/Bidaux"

d_2020_stats <- read.table("all_dendros_stats.csv", header = TRUE, sep = ";", stringsAsFactors = FALSE,  encoding = "latin1")
head(d_2020_stats)
str(d_2020_stats)

d_2020_stats$date <- as.Date(d_2020_stats$date, format = "%Y-%m-%d")

# get the file 2021______________________________________________________________________________________________________
setwd("~/mnt/Data-Work-RE/26_Agricultural_Engineering-RE/263_DP/03_Persoenliche_Unterlagen/Wata/08-R_dendro_files/Bidaux/20210911_v4/daily_data")

d_2021_stats <- read.table("all_dendros_stats.csv", header = TRUE, sep = ";", stringsAsFactors = FALSE,  encoding = "latin1")
head(d_2021_stats)
str(d_2021_stats)

d_2021_stats$date <- as.Date(d_2021_stats$date, format = "%Y-%m-%d")


# merge the dendro_daily_status from 2020 and from 2021
#----------------------------------------------------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------------------------------------------------
head(d_2021_stats)
...
all_dendro_stats <- merge(d_2021_stats,d_2020_stats, by = "dendro", all = TRUE)
all_dendro_stats <- na.omit(all_dendro_stats)
str(all_dendro_stats)

setwd("~/mnt/Data-Work-RE/26_Agricultural_Engineering-RE/263_DP/03_Persoenliche_Unterlagen/Wata/08-R_dendro_files/Bidaux")
write.csv(all_dendro_stats, file = "all_dendro_stats.csv")

# Add Clima data from 2020____________________________________________________________________________________________________________________________________________________________

setwd("~/mnt/Data-Work-RE/26_Agricultural_Engineering-RE/263_DP/03_Persoenliche_Unterlagen/Wata/08-R_dendro_files/Bidaux/2020/raw_data")
max_daily_clima_2020 <- read.table("max_daily_clima_2020.csv", header = TRUE, sep = ";", skip = 12,  stringsAsFactors = FALSE,  encoding = "latin1")
head(max_daily_clima_2020)
str(max_daily_clima_2020)

max_daily_clima_2020$date <- as.Date(max_daily_clima_2020$date, format = "%d.%m.%Y")
str(max_daily_clima_2020)

# ET0 - file____________________________________________________________________________________________________________________________________________________________
#____________________________________________________________________________________________________________________________________________________________________
#____________________________________________________________________________________________________________________________________________________________________
setwd("~/mnt/Data-Work-RE/26_Agricultural_Engineering-RE/263_DP/03_Persoenliche_Unterlagen/Wata/08-R_dendro_files/Bidaux/2020/raw_data")
ET0_2020 <- read.table("ET0_2020.csv", header = TRUE, sep = ";", skip = 12, stringsAsFactors = FALSE,  encoding = "latin1")
head(ET0_2020)
str(ET0_2020)

ET0_2020$date <- as.Date(ET0_2020$date, format = "%d.%m.%Y")
head(ET0_2020)
str(ET0_2020)

# ET0 - daily_clima____________________________________________________________________________________________________________________________________________________________
#____________________________________________________________________________________________________________________________________________________________________
#____________________________________________________________________________________________________________________________________________________________________

Et0_clima_2020 <- 
# Complete clima data____________________________________________________________________________________________________________________________
# ____________________________________________________________________________________________________________________________
# ____________________________________________________________________________________________________________________________
setwd("~/mnt/Data-Work-RE/26_Agricultural_Engineering-RE/263_DP/03_Persoenliche_Unterlagen/Wata/08-R_dendro_files/Bidaux/2020/raw_data")
clima_2020_brut <- read.table("clima_2020_brut.csv", header = TRUE, sep = ";", skip = 12, stringsAsFactors = FALSE,  encoding = "latin1")
head(clima_2020_brut)
str(clima_2020_brut)

clima_2020_brut$date <- as.Date(clima_2020_brut$date, format = "%d.%m.%Y")
head(clima_2020_brut)
str(clima_2020_brut)



# Watermarks____________________________________________________________________________________________________________________________________________________________

idir = "~/mnt/Data-Work-RE/26_Agricultural_Engineering-RE/263_DP/03_Persoenliche_Unterlagen/Wata/08-R_dendro_files/Bidaux/2020/watermarks"
odir = "~/mnt/Data-Work-RE/26_Agricultural_Engineering-RE/263_DP/03_Persoenliche_Unterlagen/Wata/08-R_dendro_files/Bidaux/2020/watermarks_aggregated"
files <- list.files(idir, pattern = '*.csv')
library(reshape2)
wm_2020_all <- NULL
# file <- files[3]
for (file in files[1:2]) {
  wm_df <- read.csv(file.path(idir, file), header = TRUE, sep = ";",skip = 12, stringsAsFactors = FALSE)
  wm_df[,1] <- as.POSIXct(wm_df[,1], format = "%d.%m.%Y")
  for (i in 2:(length(names(wm_df)))) {
    s <- names(wm_df)[i]
    names(wm_df)[i] <- paste0(substr(s, 1,3), '_', substr(s, 9,10))
  }
  new_reshaped <- wm_df %>%  
    melt(id = 'date') %>% 
    select(date, value, variable) %>% 
    separate(col = variable, into = c('Tree', 'Width'))
  
  wm_2020_all <- rbind(wm_2020_all, new_reshaped)
}

wm_2020_all <- wm_2020_all %>% select(date, Tree, value, Width)
wm_2020_all$Width <- paste0('w', wm_2020_all$Width)
write_csv(wm_2020_all, file.path(odir, 'watermarks_long_format.csv'))


wm_2020_all$Width <- as.factor(wm_2020_all$Width)

wm_2020_all_short <- wm_2020_all %>%
  group_by(Width) %>%
  mutate(row = row_number()) %>%
  tidyr::pivot_wider(names_from = Width, values_from = value) %>%
  select(-row)



write_csv(wm_2020_all_short, file.path(odir, 'watermarks_short_format.csv'))

# Watermarks with dendro from each year______________________________________________________________________________________________________________________________________________________________
#____________________________________________________________________________________________________________________________________________________________________
#____________________________________________________________________________________________________________________________________________________________________
idir = "~/mnt/Data-Work-RE/26_Agricultural_Engineering-RE/263_DP/03_Persoenliche_Unterlagen/Wata/08-R_dendro_files/Bidaux/2020/raw_data"
